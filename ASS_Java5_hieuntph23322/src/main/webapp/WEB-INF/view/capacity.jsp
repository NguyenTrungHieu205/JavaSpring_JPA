...
<div class="container">
    <form:form action="/dung-luong/add" method="post" modelAttribute="cap">
        <br/>
...
<table class="table table-dark table-striped-columns">
    ...
    <c:forEach items="${capacity}" var="capacity">
        <tr>
            ...
            <td>${capacity.code}</td>
            <td>${capacity.name}</td>
            <td>${capacity.dateCreate}</td>
            <td>${capacity.dateCorrect}</td>
            <c:if test="${capacity.status == 0}">
                <td>Online</td>
            </c:if>
            <c:if test="${capacity.status == 1}">
                <td>Offline</td>
            </c:if>
            <td>
                <a href="/dung-luong/remove/${capacity.id}" type="button"
                   class="btn btn-danger" onclick="return  confirmSubmit()">Delete</a>
                <a href="/dung-luong/detail/${capacity.id}" type="button"
                   class="btn btn-warning">Detail</a>
                <a href="/dung-luong/view-update/${capacity.id}" type="button" class="btn btn-warning">Edit</a>
            </td>
        </tr>
    </c:forEach>
    </tbody>
...
<nav aria-label="Page navigation example">
    <ul class="pagination">
        <c:if test="${pageDungLuong.number - 1 >= 0}">
            <li class="page-item">
                <a class="page-link"
                   href="/dung-luong/hien-thi?pageNo=${pageDungLuong.number - 1}">Previous</a>
            </li>
        </c:if>
        <c:if test="${pageDungLuong.number - 1 < 0}">
            <li class="page-item disabled"><span class="page-link">Previous</span></li>
        </c:if>
        <c:forEach begin="0" end="${pageDungLuong.totalPages - 1}" varStatus="page">
            <li class="page-item">
                <a class="page-link"
                   href="/dung-luong/hien-thi?pageNo=${page.index}">${page.index+1}
                </a>
            </li>
        </c:forEach>
        <c:if test="${pageDungLuong.number + 1 <= pageDungLuong.totalPages - 1}">
            <li class="page-item">
                <a class="page-link"
                   href="/dung-luong/hien-thi?pageNo=${pageDungLuong.number + 1}">Next
                </a>
            </li>
        </c:if>
        <c:if test="${pageDungLuong.number + 1 > pageDungLuong.totalPages - 1}">
